...
where employee.salary > (select avg(ein.salary) from employee as ein where ein.department_name=employee.department_name);

select employee.ename, employee.salary, employee.department_name from employee
where employee.salary >=all (select ein.salary from employee as ein where ein.department_name=employee.department_name);

select employee.ename, employee.salary, employee.department_name from employee
where employee.salary = (select min(ein.salary) from employee as ein where ein.department_name=employee.department_name);
