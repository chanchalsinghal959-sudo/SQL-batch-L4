...
select s.student_id, s.student_name, e.enrollment_id, e.course_name,
e.student_id from students as s right join enrollments as e 
on s.student_id=e.student_id;
...
select s.student_id, s.student_name, count(enrollment_id)
from students as s
left join enrollments as e 
on s.student_id=e.student_id group by student_id
order by count(enrollment_id) desc limit 1 offset 2;

select char_length(s.student_name),count(e.enrollment_
